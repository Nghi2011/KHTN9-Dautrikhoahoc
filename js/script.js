...
function update() {
    ...
    obstacles.forEach(o => {
        ...
        if (checkCollision(player, o)) {
            if (o.type === 'platform') { if (player.dy > 0) { player.y = o.y - player.height; player.dy = 0; player.grounded = true; } }
            else if (o.type === 'spring') { player.dy = -18; player.grounded = false; }
            else if (o.type === 'shield') { hasShield = true; o.x = -1000; }
            else if (o.type === 'magnet') { magnetActive = 300; o.x = -1000; }
            else if (o.type === 'spike' || o.type === 'police') { if (hasShield) { hasShield = false; o.x = -1000; } else handleDeath(); }
        }
    });

    if (currentLevel === 20) lavaHeight -= 0.45;
    if (checkCollision(player, gem)) triggerQuestion();
    if (player.y > canvas.height || (currentLevel === 20 && player.y + player.height > lavaHeight)) handleDeath();
}

function draw() {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    if (currentLevel === 20) { ctx.fillStyle = "rgba(255, 0, 0, 0.4)"; ctx.fillRect(0, lavaHeight, 900, 500); }
    
    obstacles.forEach(o => {
        ctx.fillStyle = (o.type === 'spike') ? '#ff4d4d' : (o.type === 'spring' ? '#2ecc71' : (o.type === 'shield' ? '#00d2ff' : (o.type === 'magnet' ? '#9b59b6' : '#7f8c8d')));
        ctx.fillRect(o.x, o.y, o.w, o.h);
        if (o.type === 'police') ctx.fillText("👮", o.x + 10, o.y + 25);
        if (o.type === 'spring') ctx.fillText("🚀", o.x + 10, o.y + 15);
        if (o.type === 'magnet') ctx.fillText("🧲", o.x + 5, o.y + 20);
    });

    ctx.fillStyle = "#f1c40f"; ctx.beginPath(); ctx.arc(gem.x+15, gem.y+15, 15, 0, Math.PI*2); ctx.fill();
    
    if (hasShield) { ctx.strokeStyle = "#00d2ff"; ctx.lineWidth = 3; ctx.strokeRect(player.x-2, player.y-2, player.width+4, player.height+4); }
    ctx.drawImage(playerImg, player.x, player.y, player.width, player.height);
    ctx.fillStyle = "white"; ctx.font = "bold 10px Arial"; ctx.textAlign = "center";
    ctx.fillText(playerClass, player.x + player.width/2, player.y - 5);

    if (hintTimer > 0 && gameState === 'PLAYING') {
        ctx.fillStyle = "rgba(0, 0, 0, 0.6)"; ctx.fillRect(150, 30, 600, 40);
        ctx.fillStyle = "#fff"; ctx.font = "bold 16px Arial";
        ctx.fillText("HƯỚNG DẪN: " + currentHint, canvas.width/2, 55);
        if (!isPaused) hintTimer--;
    }
}
